...
public class Model {
    ...
    public Pixmap evaluate(String input, Dimension size) {
        ...
        Map<String, RGBColor> map = new HashMap<String, RGBColor>();
        double evalT = myCurrentTime * 2 - 1;
        ...
        for (int imageY = 0; imageY < size.height; imageY++) {
            ...
            for (int imageX = 0; imageX < size.width; imageX++) {
                ...
                map.put("x", new RGBColor(evalX));
                map.put("y", new RGBColor(evalY));
                map.put("t", new RGBColor(evalT));
                result.setColor(imageX, imageY,
                        toEval.evaluate(evalX, evalY, map).toJavaColor());
            }
        }
        return result;
    }
